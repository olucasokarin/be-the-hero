...
module.exports = {
    async list(request, response) {
        ...
        .offset((page-1)*5) //calculo para pegar a proxima pagina
        .select(['incidents.*',
    ...
    async create(request, response) {
        ...
        const verification = await connection('ongs').select('name').where('id', ong_id);
        ...
        if(verification.length != 0){ 
            ...
            return response.status(401).send({error:'Id inválido'});
        }
